...
public class Elevator extends SubsystemBase {
    ...
    private TrapezoidProfile trapezoidProfile =
            new TrapezoidProfile(new Constraints(0.5, 0.9 / 60.0));
    ...
    public Elevator() {
        ...
        configLead.inverted(true).idleMode(IdleMode.kBrake)
                .smartCurrentLimit(50);
        configLead.encoder.positionConversionFactor(positionFactor)
                .velocityConversionFactor(positionFactor / 60);
        configLead.closedLoop.pid(20, 0, 0).outputRange(-0.5, 0.9);
        // .feedbackSensor(FeedbackSensor.kPrimaryEncoder);
        // configLead.closedLoop.maxMotion
        // .maxVelocity(1)
        // .maxAcceleration(1);
        elevator1.configure(configLead, ResetMode.kResetSafeParameters,
                PersistMode.kNoPersistParameters);
        SparkMaxConfig configFollow = new SparkMaxConfig();
        configFollow.inverted(false).idleMode(IdleMode.kBrake)
                .smartCurrentLimit(50);
        configFollow.encoder.positionConversionFactor(positionFactor)
                .velocityConversionFactor(positionFactor / 60);
        configFollow.closedLoop.pid(20, 0, 0).outputRange(-0.5, 0.9);
        ...
        Shuffleboard.getTab("Debug").addDouble("Current Elevator goal",
                () -> currentSetpoint);
        Shuffleboard.getTab("Debug").addDouble(
                "Current elevator motor 1 position",
                () -> elevator1.getEncoder().getPosition());
        Shuffleboard.getTab("Debug").addDouble(
                "Current elevator motor 2 position",
                () -> elevator2.getEncoder().getPosition());
        Shuffleboard.getTab("Debug").addDouble("Current elevator setpoint",
        ...
        var sysIdRoutine = new SysIdRoutine(
                ...
                new SysIdRoutine.Mechanism(
                        (voltage) -> this.runVolts(voltage.in(Volts)), null, // No log consumer, since data is recorded by URCL
                        this));
    ...
    public void periodic() {
        if (DriverStation.isDisabled()) {
            trapezoidSetpoint = new TrapezoidProfile.State(
                    elevator1.getEncoder().getPosition(),
                    elevator1.getEncoder().getVelocity());
    ...
    public Command l1() {
        return run(() -> setPosition(
                Constants.SetpointConstants.ElevatorSetpoints.l1.get()));
    }

    public Command l2() {
        return run(() -> setPosition(
                Constants.SetpointConstants.ElevatorSetpoints.l2.get()));
    }

    public Command l3() {
        return run(() -> setPosition(
                Constants.SetpointConstants.ElevatorSetpoints.l3.get()));
    }

    public Command l4() {
        return run(() -> setPosition(
                Constants.SetpointConstants.ElevatorSetpoints.l4.get()));
    }

    public Command algaeLow() {
        return run(() -> setPosition(
                Constants.SetpointConstants.ElevatorSetpoints.algaeLow.get()));
    }

    public Command algaeHigh() {
        return run(() -> setPosition(
                Constants.SetpointConstants.ElevatorSetpoints.algaeHigh.get()));
    }
