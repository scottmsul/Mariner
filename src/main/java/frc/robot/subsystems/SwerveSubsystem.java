...
public class SwerveSubsystem extends SubsystemBase {
    ...
    public void driveHuman(double xPercent, double yPercent, double rotPercent,
        ...
        velocity = ChassisSpeeds.fromFieldRelativeSpeeds(velocity,
                getRotation().minus(driverRotationOffset));
    ...
    public void driveAuto(double xPercent, double yPercent, double rotPercent,
            boolean fieldRelative) {
        var xSpeed =
                xPercent * Constants.DriveConstants.MaxVelocityMetersPerSecond;
        var ySpeed =
                yPercent * Constants.DriveConstants.MaxVelocityMetersPerSecond;
        var rot = rotPercent
                * Constants.DriveConstants.MaxAngularVelocityRadiansPerSecond;
    ...
    public void xpattern() {
        ...
        for (var i = 0; i < states.length; i++) {
            states[i].angle =
                    DriveConstants.kinematics.getModules()[i].getAngle();
    ...
    private void doMegatag(String limelight) {
        ...
        if (useMegaTag2 == false) {
            ...
            LimelightHelpers.SetRobotOrientation(limelight,
                    swerveDrive.getPose().getRotation().getDegrees(), 0, 0, 0,
                    0, 0);
    ...
    public SwerveSubsystem() {
        ...
        try {
            swerveDrive = new SwerveParser(swerveJsonDirectory)
                    .createSwerveDrive(maximumSpeedMps);
        } catch (Exception e) {
        ...
        Shuffleboard.getTab("SysId").add("Quasi Forward Swerve",
                Commands.run(() -> this.runCharacterization(0)).withTimeout(1)
        ...
        Shuffleboard.getTab("SysId").add("Dynamic Forward Swerve", Commands
                .run(() -> this.runCharacterization(0)).withTimeout(1).andThen(
                        sysIdRoutine.dynamic(SysIdRoutine.Direction.kForward)));
        Shuffleboard.getTab("SysId").add("Dynamic Backaward Swerve", Commands
                .run(() -> this.runCharacterization(0)).withTimeout(1).andThen(
    ...
    public void postTrajectory(List<Pose2d> poses) {
        ...
        for (int i = 0; i < poses.size(); i++) {
            ...
            Trajectory.State state =
                    new Trajectory.State(totalTime, velocity, 0.0, // acceleration
                            pose, 0.0 // curvature
                    );
            states.add(state);
        }
