...
public class AutoAlignReefEstimation extends Command {
    ...
    final Optional<Pose3d> getTargetPose() {
        if (LimelightHelpers.getTV(Constants.ReefLimelightName)) {
            ...
            var robotAbsPose = new Pose3d(swerveSub.getPose());
            ...
            // var targetAbsPose = Apriltags.all[lastApriltag];

            // var targetPoseRelativeToRobot = targetAbsPose.relativeTo(new
            // Pose3d(robotAbsPose));
            // return Optional.of(targetPoseRelativeToRobot);
            return Optional.empty();
        }
